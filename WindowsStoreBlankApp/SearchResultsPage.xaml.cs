using WindowsStoreBlankApp.Common;
using System;
...
namespace WindowsStoreBlankApp
{
    ...
    {
        //added in:
        private Dictionary<string, List<Customer>> searchResults = new Dictionary<string, List<Customer>>();
        ...
        //added: handles click events on results
        ...
        private void navigationHelper_LoadState(object sender, LoadStateEventArgs e)
        {
            ...
            List<Customer> allCustomers = (Application.Current as App).MainViewModel.AllCustomers;

            var filterList = new List<Filter>();
            //filterList.Add(new Filter("All", 0, true));
            //Find all customers matching firstName or lastName
            queryText = queryText.ToLower();
            List<Customer> matchingFirstNames = new List<Customer>();
            List<Customer> matchingLastNames = new List<Customer>();
            foreach (Customer customer in allCustomers)
            {
                if (customer.FirstName.ToLower().Contains(queryText))
                    matchingFirstNames.Add(customer);
                if (customer.LastName.ToLower().Contains(queryText))
                    matchingLastNames.Add(customer);
            }
            filterList.Add(new Filter("Matching First Names", matchingFirstNames.Count, false));
            filterList.Add(new Filter("Matching Last Names", matchingLastNames.Count, false));
